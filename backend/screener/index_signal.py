...
def generate_index_call(index_name, bias, oi, spot_or_fut, atr):
    ...
    # Sep 10 2026: real fix -- was qty = max(1, int(50000/entry)), an
    # arbitrary capital-per-trade guess with no relationship to the
    # actual exchange lot size. Uses the same real, live lot-size
    # resolver lot_size_resolver.py already provides (daily-cached
    # against Fyers' own symbol master, same function views.py's stock
    # engine already uses for the same purpose). Fails safely -- if
    # the real lot size can't be resolved, this returns None rather
    # than inventing a quantity, same "never guess a number" principle
    # already used everywhere else in this function (missing premium/
    # delta/strike all return None above, not a placeholder).
