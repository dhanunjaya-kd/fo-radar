...
function AppShell() {
  ...
  useEffect(() => {
    try { localStorage.setItem('fo-radar-active-tab', activeTab); } catch {}
  }, [activeTab]);
  ...
  const TabButton = ({ tab }) => (
    <button
      ...
      className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-slate-700 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
    >
      {tabIcon(tab.id)} {tab.label}
      {tab.count !== null && (
        <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-blue-500 text-white' : 'bg-slate-700 text-slate-300'}`}>
          {tab.count}
        </span>
      )}
    </button>
    ...
    <div className={`min-h-screen bg-slate-950 text-white overflow-x-hidden ${theme === 'light' ? 'light' : ''} density-${density}`}>
      <div className="px-4 pt-4 pb-3 flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="font-bold text-white text-sm sm:text-base whitespace-nowrap">F&amp;O SNIPER <span className="text-slate-500 font-normal">SCANNER v3.0</span></span>
        </div>
        <div className="flex items-center gap-1.5 ml-auto">
          <ThemeToggle />
          <button onClick={enableAlerts} disabled={alertsEnabled} title={alertsEnabled ? 'Alerts on' : 'Enable alerts'} aria-label={alertsEnabled ? 'Alerts on' : 'Enable alerts'} className={`w-9 h-9 flex items-center justify-center rounded-lg border transition-colors ${alertsEnabled ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/25 cursor-default' : 'text-amber-400 bg-amber-500/10 border-amber-500/25 hover:bg-amber-500/20'}`}>
